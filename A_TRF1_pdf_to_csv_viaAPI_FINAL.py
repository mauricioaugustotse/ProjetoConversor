...
# -*- coding: utf-8 -*-
"""
Orquestra o pipeline final do TRF1 (PDF -> CSV enriquecido).
...
import B_TRF1_pdf_to_csv_viaAPI_openai as core
...
def add_final_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument("--classe-config-file", default=str(SCRIPT_DIR / "canon_config" / "classe_canonical.yaml"))

    parser.add_argument("--report-dir", default=str(SCRIPT_DIR / "reports"))
    ...
    parser.add_argument("--ramo-subramo-cache-file", default=str(SCRIPT_DIR / ".checkpoint" / "ramo_subramo_openai_cache.json"))
...
def resolve_paths(args: argparse.Namespace) -> Tuple[Path, Path, Path, Path, Path, Path]:
    ...
    output_csv = output_dir / Path(args.output_name or DEFAULT_OUTPUT_NAME).name
    checkpoint_path = (
        output_dir / Path(args.checkpoint_file).name
        if (args.checkpoint_file or "").strip()
        else output_dir / (args.checkpoint_name or DEFAULT_CHECKPOINT_NAME)
    )
    quality_report_path = (
        output_dir / Path(args.quality_report_file).name
        if (args.quality_report_file or "").strip()
        else output_dir / (args.quality_report_name or DEFAULT_QUALITY_REPORT_NAME)
    )

    report_dir = output_dir / Path(args.report_dir or "reports").name
    report_dir.mkdir(parents=True, exist_ok=True)

    backup_dir = (output_dir / Path(args.backup_dir).name) if (args.backup_dir or "").strip() else output_dir
    backup_dir.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    logger = core.setup_logger(args.verbose, args.quiet, args.debug, args.log_file)
