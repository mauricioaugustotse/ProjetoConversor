...
# -*- coding: utf-8 -*-
"""
TRF1 PDF -> CSV com enriquecimento Perplexity (modo exclusivo).
...
- Gerar CSV dedicado do pipeline Perplexity.
- Enriquecer a coluna de notícia via Perplexity.
...
OUTPUT_NAME = "boletins_de_jurisprudencia_TRF1_perplexity.csv"
CHECKPOINT_NAME = ".boletins_de_jurisprudencia_TRF1_perplexity.checkpoint.json"
QUALITY_REPORT_NAME = ".boletins_de_jurisprudencia_TRF1_perplexity.quality_report.json"
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="TRF1 PDF para CSV Perplexity-only (retoma checkpoint compatível).",
    )
...
def setup_logger(verbose: bool, quiet: bool) -> logging.Logger:
    logger = logging.getLogger("trf1_perplexity")
...
def main() -> None:
    ...
    ensure_output_rows_format(rows)
    pplx_checkpoint = summarize_perplexity_progress(rows, args.perplexity_skip_terminal_reasons)
    pplx_checkpoint_pct = (
        (pplx_checkpoint["effective_done"] / pplx_checkpoint["total_rows"] * 100.0)
        if pplx_checkpoint["total_rows"]
        else 100.0
    )
    logger.info(
        (
            "Checkpoint Perplexity: total=%d | com_url=%d | encerradas_terminal=%d | "
            "pendentes_reais=%d | progresso=%.2f%%"
        ),
        pplx_checkpoint["total_rows"],
        pplx_checkpoint["done_with_url"],
        pplx_checkpoint["skipped_terminal"],
        pplx_checkpoint["pending_retryable"],
        pplx_checkpoint_pct,
    )
    ...
    total = len(rows)
    pplx_summary = summarize_perplexity_progress(rows, args.perplexity_skip_terminal_reasons)
    logger.info(
        "Concluído. Linhas: %d | Perplexity com URL: %d | Perplexity terminais: %d | Perplexity pendentes reais: %d",
        total,
        pplx_summary["done_with_url"],
        pplx_summary["skipped_terminal"],
        pplx_summary["pending_retryable"],
    )
