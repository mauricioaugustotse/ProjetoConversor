...
# -*- coding: utf-8 -*-
"""
Detecta acréscimos em Temas Selecionados e gera CSV complementar.

Fluxo de trabalho:
1. Carrega CSV base e mapeia fontes HTML locais e páginas live por ramo.
2. Extrai candidatos de julgados, normaliza chaves e remove itens já existentes.
3. Separa incompletos para auditoria e prepara apenas novos itens elegíveis.
4. Enriquece `tema/contexto/tese/punchline/bullet_points` via OpenAI com checkpoint.
5. Gera CSV complementar e arquivos de auditoria (novos, descartados, incompletos e falhas).
"""
...
# O script residia na raiz do projeto; SCRIPT_DIR sempre significou a raiz
# (base para inputs/saidas relativos, glob de HTMLs e pasta inicial da GUI).
...
LOGGER = logging.getLogger("TEMAS_SELC_COMPLETAR_txt_to_csv_")
...
def force_output_path(path_value: str | Path) -> Path:
    name = Path(path_value).name
    if not name:
        name = "temas_selec_TSE_complementar.csv"
...
def retry_wait_seconds(attempt: int, retry_wait_s: float, error_kind: str) -> float:
    base = max(0.2, float(retry_wait_s))
    if error_kind in {"rate_limit", "timeout", "connection", "upstream"}:
        base = max(base, 0.8)
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if not retryable:
                break
        if attempt < retries:
            time.sleep(retry_wait_seconds(attempt, retry_wait_s, last_kind))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--html-files", nargs="*", default=[])
    parser.add_argument("--csv-out", default="temas_selec_TSE_complementar.csv")
    parser.add_argument("--audit-prefix", default="")
    parser.add_argument("--model", "--openai-model", dest="model", default="gpt-5-mini")
    parser.add_argument("--workers", "--openai-max-workers", dest="workers", type=int, default=OPENAI_DEFAULT_MAX_WORKERS)
    parser.add_argument(
        "--max-workers-cap",
        "--openai-max-workers-cap",
        dest="max_workers_cap",
        type=int,
        default=OPENAI_DEFAULT_MAX_WORKERS_CAP,
    )
    parser.add_argument("--batch-size", "--openai-batch-size", dest="batch_size", type=int, default=OPENAI_DEFAULT_BATCH_SIZE)
    parser.add_argument("--delay", "--openai-delay", dest="delay", type=float, default=OPENAI_DEFAULT_DELAY)
    parser.add_argument("--retries", "--openai-retries", dest="retries", type=int, default=OPENAI_DEFAULT_RETRIES)
    parser.add_argument("--retry-wait-s", "--openai-retry-wait-s", dest="retry_wait_s", type=float, default=1.0)
    parser.add_argument("--timeout", "--openai-timeout", dest="timeout", type=int, default=OPENAI_DEFAULT_TIMEOUT)
    parser.add_argument("--target-rpm", "--openai-target-rpm", dest="target_rpm", type=int, default=OPENAI_DEFAULT_TARGET_RPM)
    parser.add_argument("--save-every", type=int, default=20)
    parser.add_argument("--checkpoint", default="")
    parser.add_argument("--temas-script", default="conversores/TEMAS_SELC_txt_to_csv.py")
...
def maybe_collect_gui_inputs(args: argparse.Namespace) -> argparse.Namespace:
    ...
    explicit_inputs = bool(args.html_files) or any(
        tok in ("--base-csv", "--html-glob")
        or tok.startswith("--base-csv=")
        or tok.startswith("--html-glob=")
        for tok in cli_tokens
    )
    if args.no_gui or explicit_inputs:
        return args

    csv_out_path = force_output_path(args.csv_out)
    initial_output_dir = str(SCRIPT_DIR)
    gui = open_file_panel(
        title="Temas Selecionados Complementar - Painel",
        subtitle="Selecione HTMLs/csv base e configure a execucao.",
        filetypes=[("HTML e CSV", "*.html *.htm *.csv"), ("Todos os arquivos", "*.*")],
        ...
        extra_bools=[
            ("use_live_fallback", "Usar fallback live", bool(args.use_live_fallback)),
            ("verbose", "Logs detalhados", bool(args.verbose)),
            ("quiet", "Apenas avisos/erros", bool(args.quiet)),
            ("debug", "Debug técnico", bool(args.debug)),
        ],
        extra_texts=[
            ("base_csv", "CSV base", str(args.base_csv)),
            ("output_name", "Nome do CSV de saida", csv_out_path.name or "temas_selec_TSE_complementar.csv"),
            ("audit_prefix", "Prefixo de auditoria", str(args.audit_prefix or "")),
            ("model", "Modelo OpenAI", str(args.model)),
            ("workers", "Workers", str(args.workers)),
            ("batch_size", "OpenAI batch size", str(args.batch_size)),
            ("delay", "OpenAI delay entre lotes (s)", str(args.delay)),
            ("html_glob", "Glob HTML (fallback quando nenhum HTML for selecionado)", str(args.html_glob)),
    ...
    files = dedupe_files(gui.get("files") or [])
    html_files = [f for f in files if f.lower().endswith((".html", ".htm"))]
    csv_files = [f for f in files if f.lower().endswith(".csv")]
    texts = gui.get("texts") or {}

    base_csv_from_text = str(texts.get("base_csv", "")).strip()
    if base_csv_from_text:
        args.base_csv = base_csv_from_text
    elif csv_files:
        args.base_csv = csv_files[0]

    if html_files:
        args.html_files = html_files
    else:
        args.html_glob = str(texts.get("html_glob", args.html_glob)).strip() or args.html_glob

    output_name = str(texts.get("output_name", "")).strip() or csv_out_path.name
    args.csv_out = str(force_output_path(output_name))

    args.audit_prefix = str(texts.get("audit_prefix", args.audit_prefix)).strip()
    args.model = str(texts.get("model", args.model)).strip() or args.model
    try:
        args.workers = max(1, int(str(texts.get("workers", args.workers)).strip() or args.workers))
    except Exception:
        pass
    try:
        args.batch_size = max(1, int(str(texts.get("batch_size", args.batch_size)).strip() or args.batch_size))
    except Exception:
        pass
    try:
        args.delay = max(0.0, float(str(texts.get("delay", args.delay)).strip() or args.delay))
    except Exception:
        pass
    bools = gui.get("bools") or {}
    args.use_live_fallback = bool(bools.get("use_live_fallback", args.use_live_fallback))
...
def main() -> None:
    ...
    logger = configure_standard_logging(
        "TEMAS_SELC_COMPLETAR_txt_to_csv_",
    ...
    base_csv = resolve_input_path(args.base_csv)
    csv_out = force_output_path(args.csv_out)
    if args.html_files:
        html_paths = sorted(
            resolve_input_path(p)
            for p in args.html_files
            if resolve_input_path(p).exists()
        )
    else:
        html_paths = sorted(SCRIPT_DIR.glob(args.html_glob))
    audit_prefix = build_audit_prefix(args.audit_prefix)
    checkpoint = force_output_path(args.checkpoint) if args.checkpoint else force_output_path(f"{csv_out.name}.checkpoint.json")
    report_json = force_output_path(f"{audit_prefix}_resumo.json")
    ...
    source_sig = build_file_signature(base_csv)

    if csv_out.exists():
        backup = make_backup(csv_out, label="startup_backup")
        if backup is not None:
            print(f"[backup] snapshot inicial criado: {backup}")

    prompt_base = load_prompt_from_temas_script(resolve_input_path(args.temas_script))
    ...
    if not rows_out:
        write_csv_atomic(csv_out, rows_out, fieldnames)
        write_dict_csv(
            force_output_path(f"{audit_prefix}_novos_detectados.csv"),
            [],
            [
                "row_idx_out",
                "ID",
        ...
        write_dict_csv(
            force_output_path(f"{audit_prefix}_ja_existentes_descartados.csv"),
            descartados,
            ["ramo", "numero_processo", "tipo_de_processo", "data_julgamento", "href", "source", "motivo"],
        )
        write_dict_csv(
            force_output_path(f"{audit_prefix}_incompletos_sem_chave.csv"),
            incompletos,
            [
                "ramo",
                "href",
        ...
        write_dict_csv(
            force_output_path(f"{audit_prefix}_api_falhas.csv"),
            [],
        ...
        force_output_path(f"{audit_prefix}_resumo.txt").write_text("\n".join(summary) + "\n", encoding="utf-8")
        write_json_atomic(
            report_json,
            {
                "script": "TEMAS_SELC_COMPLETAR_txt_to_csv_.py",
                "status": "completed_empty",
                "csv_base": str(base_csv.resolve()),
                "csv_saida": str(csv_out.resolve()),
                "source_signature": source_sig,
                "novos_detectados": 0,
    ...
    workers = min(max(1, int(args.workers)), max(1, int(args.max_workers_cap)))
    batch_size = max(1, int(args.batch_size))
    batch_delay = max(0.0, float(args.delay))
    pacer = RequestPacer(max(0, int(args.target_rpm)))

    pending = [i for i in range(len(rows_out)) if i not in done_indices]
    print(
        f"[api] pendentes: {len(pending)} / {len(rows_out)} | workers={workers} "
        f"| batch={batch_size} | delay={batch_delay:.2f}s | rpm={int(args.target_rpm)}"
    )
    lock = threading.Lock()
    since_save = 0
    ...
    for start in range(0, len(pending), batch_size):
        ...
        batch_indices = pending[start:end]
        print(f"[api] lote {start + 1}-{end}/{len(pending)}")
        with ThreadPoolExecutor(max_workers=min(workers, len(batch_indices))) as ex:
            fut_map = {ex.submit(_worker, idx): idx for idx in batch_indices}
        ...
        if end < len(pending) and batch_delay > 0:
    ...
    # auditorias finais
    novos_path = force_output_path(f"{audit_prefix}_novos_detectados.csv")
    desc_path = force_output_path(f"{audit_prefix}_ja_existentes_descartados.csv")
    inc_path = force_output_path(f"{audit_prefix}_incompletos_sem_chave.csv")
    fail_path = force_output_path(f"{audit_prefix}_api_falhas.csv")
    resumo_path = force_output_path(f"{audit_prefix}_resumo.txt")
    ...
    write_json_atomic(
        report_json,
        {
            ...
            "csv_saida": str(csv_out.resolve()),
            "source_signature": source_sig,
            "novos_detectados": len(novos_detectados),
            "descartados": len(descartados),
            "incompletos": len(incompletos),
            "api_sucesso": ok_count,
            "api_falhas": fail_count,
            "source_stats": source_stats,
            "auditorias": {
                "novos": str(novos_path),
                "descartados": str(desc_path),
                "incompletos": str(inc_path),
                "api_falhas": str(fail_path),
