...
# -*- coding: utf-8 -*-
"""
Revisa a coluna `tema` em CSVs do TRF1 usando OpenAI.

Fluxo de trabalho:
1. Resolve CSV de entrada (CLI/GUI), saída, backup e checkpoint.
2. Carrega linhas, garante coluna `tema` e reaplica progresso salvo quando compatível.
3. Seleciona pendências (`invalid_only` ou `all`) e revisa `tema` em lotes via OpenAI.
4. Usa perfis de insumo com fallback, retries e controle de taxa para estabilidade.
5. Salva snapshots/checkpoint ao longo do processo e escreve o CSV revisado.
"""
...
# Antes do movimento para conversores/, este diretorio era a raiz do projeto;
# mantem-se a ancoragem na raiz para preservar saidas e caminhos relativos.
...
def normalize_dates_in_rows(rows: List[Dict[str, str]], *, prefer_day_first: bool = False) -> None:
    for row in rows:
...
def force_output_path(path_str: str, *, default_name: str) -> Path:
    name = Path(path_str).name if normalize_ws(path_str) else ""
    if not name:
        name = default_name
...
def setup_logger(verbose: bool, quiet: bool, debug: bool = False, log_file: str = "") -> logging.Logger:
    return configure_standard_logging(
        "C_TRF1_csv_revisar_tema_viaAPI_openai",
...
def maybe_collect_gui_inputs(args: argparse.Namespace) -> argparse.Namespace:
    input_csv_cli = normalize_ws(args.input_csv)
    if args.no_gui or input_csv_cli:
        return args

    output_csv_current = force_output_path(
        args.output_csv,
        default_name=f"saida{DEFAULT_OUTPUT_SUFFIX}.csv",
    )
    gui = open_file_panel(
        title="TRF1 Revisao de Tema - CSV",
        subtitle="Selecione o CSV de entrada e configure a execucao.",
        filetypes=[("CSV", "*.csv"), ("Todos os arquivos", "*.*")],
        extensions=[".csv"],
        initial_files=[],
        allow_add_dir=True,
        ...
        initial_output=str(output_csv_current.parent.resolve()),
        extra_bools=[
            ("inplace", "Sobrescrever CSV de entrada (inplace)", bool(args.inplace)),
            ("no_backup", "Desativar backup", bool(args.no_backup)),
            ("reprocess_all", "Reprocessar todos os temas", str(args.reprocess_mode) == "all"),
            ("verbose", "Verbose", bool(args.verbose)),
            ("quiet", "Quiet", bool(args.quiet)),
        ],
        extra_texts=[
            ("output_name", "Nome do CSV de saida", output_csv_current.name),
            ("checkpoint_name", "Nome do checkpoint", Path(args.checkpoint_file).name if normalize_ws(args.checkpoint_file) else ""),
    ...
    args.inplace = bool(bools.get("inplace", args.inplace))
    args.no_backup = bool(bools.get("no_backup", args.no_backup))
    args.verbose = bool(bools.get("verbose", args.verbose))
    args.quiet = bool(bools.get("quiet", args.quiet))
    args.reprocess_mode = "all" if bool(bools.get("reprocess_all", False)) else "invalid_only"
    ...
    if args.inplace:
        ...
        output_name = str(texts.get("output_name", "")).strip() or output_csv_current.name
        args.output_csv = str(force_output_path(output_name, default_name=output_csv_current.name))

    checkpoint_name = str(texts.get("checkpoint_name", "")).strip()
    if checkpoint_name:
        checkpoint_base_dir = (
            resolve_project_path(args.output_csv).parent
            if normalize_ws(args.output_csv)
            else resolve_project_path(args.input_csv).parent
        )
        args.checkpoint_file = str(checkpoint_base_dir / checkpoint_name)
...
def default_output_path(input_csv: Path) -> Path:
    return SCRIPT_DIR / f"{input_csv.stem}{DEFAULT_OUTPUT_SUFFIX}{input_csv.suffix}"
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--verbose", action="store_true", help="Exibe logs detalhados.")
    p.add_argument("--quiet", action="store_true", help="Exibe apenas avisos/erros.")
...
def main() -> None:
    ...
    input_csv = resolve_project_path(input_csv_str)
    ...
    if args.inplace:
        ...
        output_csv = force_output_path(
            args.output_csv,
            default_name=default_output_path(input_csv).name,
        )
    else:
    ...
    checkpoint_path = (
        force_output_path(
            ...
            default_name=default_checkpoint_path(input_csv, output_csv).name,
        )
    ...
    if normalize_ws(args.backup_dir):
        backup_dir = SCRIPT_DIR / Path(args.backup_dir).name
    ...
    normalize_dates_in_rows(rows, prefer_day_first=False)
