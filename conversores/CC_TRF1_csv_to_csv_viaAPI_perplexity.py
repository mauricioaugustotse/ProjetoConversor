"""
Enriquece CSV do TRF1 com links de notícias via Perplexity.

Fluxo de trabalho:
1. Lê o CSV de entrada e normaliza colunas de notícia legadas/atuais.
2. Restaura cache e checkpoint para retomar sem retrabalho.
3. Seleciona linhas sem notícia, monta consultas e chama a API em lotes assíncronos.
4. Valida/deduplica URLs retornadas e preenche `noticia_1` e `noticia_2`.
5. Persiste snapshots durante a execução e grava o CSV final.
"""
...
# === CONFIGURACAO ===
# SCRIPT_DIR sempre representou a raiz do projeto (o script residia na raiz);
# apos o movimento para conversores/, segue ancorado na raiz via _PROJECT_ROOT.
SCRIPT_DIR = _PROJECT_ROOT
INPUT_FILE = str(SCRIPT_DIR / "boletins_de_jurisprudencia_TRF1.csv")
OUTPUT_FILE = str(SCRIPT_DIR / "boletins_de_jurisprudencia_TRF1_with_news.csv")
CACHE_FILE = str(SCRIPT_DIR / "perplexity_cache.json")
CHECKPOINT_FILE = str(SCRIPT_DIR / ".boletins_de_jurisprudencia_TRF1_with_news.checkpoint.json")
CHECKPOINT_VERSION = 2
PERPLEXITY_API_KEY_OVERRIDE = ""
...
DATE_COL_KEYS = {"data_julgamento", "data_publicacao", "data_decisao", "datadecisao"}
...
def normalize_date_to_mdy(value: Any, *, prefer_day_first: bool = False) -> str:
...
def normalize_row_dates(row: Dict[str, str], *, prefer_day_first: bool = False) -> None:
    for key in list(row.keys()):
        if key.lower() in DATE_COL_KEYS:
            row[key] = normalize_date_to_mdy(row.get(key, ""), prefer_day_first=prefer_day_first)
...
def force_output_path(path_str: str) -> Path:
...
def resolve_perplexity_key(cli_value: str) -> str:
    ...
    if env:
        return env
    for fallback in (SCRIPT_DIR / "Chave_secreta_Perplexity.txt", Path("Chave_secreta_Perplexity.txt")):
        val = read_secret_from_file(str(fallback))
        if val:
            return val
    return ""
...
def write_output_csv(rows: List[Dict[str, str]], fieldnames: List[str]) -> None:
    ...
    with open(tmp_path, "w", encoding="utf-8", newline="") as f:
        ...
        writer.writeheader()
        for row in rows:
            normalize_row_dates(row, prefer_day_first=False)
...
async def fill_noticia_column() -> None:
    api_key = resolve_perplexity_key(PERPLEXITY_API_KEY_OVERRIDE)
    if not api_key:
        raise SystemExit(
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="TRF1 CSV -> CSV com noticias via Perplexity.")
    p.add_argument("--input-csv", default=INPUT_FILE, help="CSV de entrada.")
    p.add_argument("--output-csv", default=OUTPUT_FILE, help="CSV de saida.")
    p.add_argument("--cache-file", default=CACHE_FILE, help="Arquivo de cache JSON.")
    p.add_argument("--checkpoint-file", default=CHECKPOINT_FILE, help="Arquivo de checkpoint JSON.")
    p.add_argument("--perplexity-api-key", default="", help="API key Perplexity.")
    p.add_argument("--no-gui", action="store_true", help="Desativa painel GUI e usa apenas CLI.")
    return p


def maybe_collect_gui_inputs(args: argparse.Namespace) -> argparse.Namespace:
    cli_tokens = list(sys.argv[1:])
    input_explicit = any(tok == "--input-csv" or tok.startswith("--input-csv=") for tok in cli_tokens)
    if args.no_gui or input_explicit:
        return args

    current_output = force_output_path(args.output_csv)
    initial_output_dir = str(SCRIPT_DIR)
    gui = open_file_panel(
        title="TRF1 CSV -> CSV via Perplexity",
        subtitle="Selecione o CSV de entrada e configure saida/checkpoint.",
        ...
        extra_texts=[
            ("output_name", "Nome do CSV de saida", current_output.name or "output_with_news.csv"),
            ("cache_name", "Nome do arquivo de cache", Path(args.cache_file).name),
            ("checkpoint_name", "Nome do checkpoint", Path(args.checkpoint_file).name),
            ("api_key", "Perplexity API key (opcional)", args.perplexity_api_key or ""),
        ],
    )
    ...
    if not files:
        raise SystemExit("Nenhum CSV selecionado no painel GUI.")

    texts = gui.get("texts") or {}
    output_name = str(texts.get("output_name", "")).strip() or current_output.name
    cache_name = str(texts.get("cache_name", "")).strip() or Path(args.cache_file).name
    checkpoint_name = str(texts.get("checkpoint_name", "")).strip() or Path(args.checkpoint_file).name
    api_key = str(texts.get("api_key", "")).strip()

    args.input_csv = files[0]
    args.output_csv = str(force_output_path(output_name))
    args.cache_file = str(force_output_path(cache_name))
    args.checkpoint_file = str(force_output_path(checkpoint_name))
    args.perplexity_api_key = api_key or args.perplexity_api_key
    return args


def main() -> None:
    global INPUT_FILE, OUTPUT_FILE, CACHE_FILE, CHECKPOINT_FILE, PERPLEXITY_API_KEY_OVERRIDE

    args = build_arg_parser().parse_args()
    args = maybe_collect_gui_inputs(args)

    INPUT_FILE = str(resolve_project_path(args.input_csv))
    OUTPUT_FILE = str(force_output_path(args.output_csv))
    CACHE_FILE = str(force_output_path(args.cache_file))
    CHECKPOINT_FILE = str(force_output_path(args.checkpoint_file))
    PERPLEXITY_API_KEY_OVERRIDE = str(args.perplexity_api_key or "").strip()

    input_path = Path(INPUT_FILE)
    if not input_path.exists() or not input_path.is_file():
        raise SystemExit(f"CSV de entrada nao encontrado: {input_path}")
