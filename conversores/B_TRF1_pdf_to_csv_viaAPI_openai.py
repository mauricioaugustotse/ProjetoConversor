...
# -*- coding: utf-8 -*-
"""
Extrai boletins do TRF1 de PDF para CSV com enriquecimento OpenAI.

Fluxo de trabalho:
1. Descobre PDFs de entrada (CLI/GUI) e valida dependências/chaves.
2. Extrai julgados com parser determinístico e normaliza campos principais.
3. Deduplica registros e cria snapshot inicial do CSV.
4. Enriquece colunas analíticas via OpenAI em lotes com retries e controle de taxa.
5. Mantém checkpoint/relatório de qualidade e grava o CSV final.
"""
...
# apos a mudanca para conversores/, reutiliza a raiz calculada no bootstrap.
SCRIPT_DIR = _PROJECT_ROOT
OUTPUT_NAME = "boletins_de_jurisprudencia_TRF1_openai.csv"
CHECKPOINT_NAME = ".boletins_de_jurisprudencia_TRF1_openai.checkpoint.json"
QUALITY_REPORT_NAME = ".boletins_de_jurisprudencia_TRF1_openai.quality_report.json"
...
def resolve_openai_key(cli_value: str) -> str:
    ...
    if env:
        return env
    for fallback in (SCRIPT_DIR / "Chave_secreta_OpenAI.txt", Path("Chave_secreta_OpenAI.txt"), SCRIPT_DIR / "CHAVE_SECRETA_API_Mauricio_local.txt", Path("CHAVE_SECRETA_API_Mauricio_local.txt")):
...
def summarize_openai_progress(rows: Sequence[Dict[str, str]]) -> Dict[str, int]:
    total_rows = len(rows)
    pending_retryable = sum(1 for row in rows if needs_openai(row))
    done = max(0, total_rows - pending_retryable)
    return {
...
def summarize_perplexity_progress(
    rows: Sequence[Dict[str, str]],
    skip_terminal_reasons: bool,
) -> Dict[str, int]:
    total_rows = len(rows)
    done_with_url = 0
    missing_noticia = 0
    ...
    for row in rows:
        if needs_perplexity(row):
            missing_noticia += 1
            if should_skip_perplexity_row(row, skip_terminal_reasons):
                skipped_terminal += 1
            else:
                pending_retryable += 1
        else:
    ...
    effective_done = done_with_url + skipped_terminal
    return {
        "total_rows": total_rows,
        "done_with_url": done_with_url,
        "missing_noticia": missing_noticia,
...
def maybe_collect_gui_inputs(args: argparse.Namespace) -> argparse.Namespace:
    if bool(args.no_gui) or list(args.input_dirs or []) or list(args.input_files or []):
        return args

    output_dir_default = str(SCRIPT_DIR)
    gui = open_file_panel(
        title="TRF1 PDF -> CSV OpenAI",
        subtitle="Selecione PDFs/pastas e configure as opcoes principais.",
        filetypes=[("PDF", "*.pdf"), ("Todos os arquivos", "*.*")],
        extensions=[".pdf"],
        initial_files=[],
        allow_add_dir=True,
        ...
        initial_output=output_dir_default,
        extra_bools=[
            ("verbose", "Verbose", bool(args.verbose)),
            ("quiet", "Quiet", bool(args.quiet)),
            ("disable_openai", "Desativar etapa OpenAI", bool(args.disable_openai)),
            ("disable_openai_tema_refine", "Desativar geracao principal de tema", bool(args.disable_openai_tema_refine)),
        ],
        extra_texts=[
            ("openai_model", "Modelo OpenAI", str(args.openai_model)),
    ...
    args.disable_openai = bool(bools.get("disable_openai", args.disable_openai))
    args.disable_openai_tema_refine = bool(
        bools.get("disable_openai_tema_refine", args.disable_openai_tema_refine)
    )
...
def run_perplexity_enrichment(
    ...
    progress = summarize_perplexity_progress(rows, config.skip_terminal_reasons)
    ...
    logger.info(
        ...
        progress["total_rows"],
        progress["done_with_url"],
        progress["skipped_terminal"],
        progress["pending_retryable"],
        progress_pct,
    )
    if not pending:
        logger.info(
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="TRF1 PDF para CSV OpenAI-only (retoma checkpoint compatível).",
    )
    ...
    p.add_argument("--output-dir", default=str(SCRIPT_DIR), help="Pasta de saída do CSV/checkpoint.")

    p.add_argument("--verbose", action="store_true", help="Log detalhado.")
    p.add_argument("--quiet", action="store_true", help="Reduz logs.")
    p.add_argument("--debug", action="store_true", help="Ativa modo debug técnico.")
...
def setup_logger(verbose: bool, quiet: bool, debug: bool = False, log_file: str = "") -> logging.Logger:
    return configure_standard_logging(
        "B_TRF1_pdf_to_csv_viaAPI_openai",
...
def main() -> None:
    ...
    logger = setup_logger(args.verbose, args.quiet, args.debug, args.log_file)
    ...
    ensure_output_rows_format(rows)
    openai_checkpoint = summarize_openai_progress(rows)
    openai_checkpoint_pct = (
        (openai_checkpoint["done"] / openai_checkpoint["total_rows"] * 100.0) if openai_checkpoint["total_rows"] else 100.0
    )
    logger.info(
        "Checkpoint OpenAI: total=%d | completas=%d | pendentes_reais=%d | progresso=%.2f%%",
        openai_checkpoint["total_rows"],
        openai_checkpoint["done"],
        openai_checkpoint["pending_retryable"],
        openai_checkpoint_pct,
    )
    ...
    logger.info(
        "Métricas-chave: tema_curto=%s | openai_done=%s",
    ...
    total = len(rows)
    openai_summary = summarize_openai_progress(rows)
    logger.info(
        "Concluído. Linhas: %d | OpenAI completas: %d | OpenAI pendentes reais: %d",
        total,
        openai_summary["done"],
        openai_summary["pending_retryable"],
    )
