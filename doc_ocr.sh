#!/bin/bash
# OCR dos PDFs escaneados (sem texto) de Documentos.
export LC_ALL=C
...
awk -F'\t' '$2<60 && tolower($1) ~ /\.pdf$/ {print $1}' "$OUT/extraido.tsv" | sort -u > "$OUT/ocr_todo.txt"
...
worker(){
  local wid="$1"; local lst="$2"; local tmp="/tmp/docr_w${wid}"
...
wait
echo "FIM OCR. Linhas: $(wc -l < "$OUT/ocr.tsv") | com texto: $(awk -F'\t' '$2>=60' "$OUT/ocr.tsv"|wc -l)"
