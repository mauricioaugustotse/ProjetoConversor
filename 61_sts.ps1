# Fase 3: distribui soltos de STs em subpastas por tipo. Pareia por ID. Sem hardcode de acentos.
...
$stsPath=$sts.FullName
function Sanitize($s){ ($s -replace '[\\/:\*\?"<>\|]',' ' -replace '\s{2,}',' ').Trim().TrimEnd('.',' ') }
$map=@{}; foreach($m in (Import-Csv "$out\map_sts.csv" -Encoding UTF8)){ $map[[int]$m.ID]=$m }
$cls=@{}; foreach($r in (Import-Csv "$out\lotes_sts_out\res_00.csv" -Delimiter '|' -Encoding UTF8)){ if($r.id -match '^\d+$'){ $cls[[int]$r.id]=$r.subpasta } }
$plan=@()
foreach($id in ($map.Keys|Sort-Object)){
  if(-not $cls.ContainsKey($id)){ continue }
  $sub=Sanitize $cls[$id]; if(-not $sub){ continue }
  $plan+=[pscustomobject]@{ De=$map[$id].Full; DestDir=(Join-Path $stsPath $sub); Nome=$map[$id].Nome; Sub=$sub }
}
Write-Output ("Arquivos: {0} | subpastas: {1}" -f $plan.Count, @($plan|Group-Object Sub).Count)
$plan|Group-Object Sub|Sort-Object Count -Descending|ForEach-Object{ Write-Output ("   {0,2}  {1}" -f $_.Count,$_.Name) }
if($Execute){
  $ts=Get-Date -Format 'yyyyMMdd_HHmmss'; $log="$out\log_sts_$ts.csv"
